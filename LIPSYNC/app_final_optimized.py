"""
LIPSYNC OPTIMIZED app.py
========================
Uses inference_optimized.py with:
- FP16 inference (--fp16)
- Optional seamlessClone bypass (--no-seamless) - saves ~4.4s

Config options in config.json:
  "lip_sync": {
    "use_fp16": true,           // Enable FP16 (default: true)
    "use_seamless_clone": true  // Enable seamlessClone (default: true, set false for +4s speed)
  }
"""
...
def _pipeline_settings() -> dict[str, Any]:
    ...
    return {
        ...
        "use_fp16": bool(_cfg_get(cfg, "lip_sync", "use_fp16", default=True)),
        "use_seamless_clone": bool(_cfg_get(cfg, "lip_sync", "use_seamless_clone", default=True)),
    }
...
def _run_sadtalker_optimized(
    ...
    """
    Run SadTalker with OPTIMIZED inference_optimized.py
    Falls back to inference.py if optimized version not found.
    """
    _ensure_repo(sadtalker_dir, "SadTalker")
    _ensure_dir(output_dir)
    
    # Check if optimized version exists
    optimized_inference = sadtalker_dir / "inference_optimized.py"
    use_optimized = optimized_inference.exists()
    ...
    def run_with_enhancer(enhancer_value: str) -> Optional[str]:
        ...
        try:
            print(f"[LIPSYNC] Running {'OPTIMIZED' if use_optimized else 'original'} SadTalker...", flush=True)
            print(f"[LIPSYNC] FP16={use_fp16}, seamlessClone={use_seamless_clone}", flush=True)
            print(f"[LIPSYNC] Cmd: {cmd[0]} {cmd[1]}", flush=True)
            return _run_command(cmd, sadtalker_dir, _prepare_env(use_cuda))
        except RuntimeError as exc:
            return str(exc)

    output = run_with_enhancer(enhancer_arg)
    print(f"[LIPSYNC] SadTalker completed, looking for output in {output_dir}", flush=True)
    result = _pick_latest_mp4(output_dir)
    if result:
        print(f"[LIPSYNC] Found result: {result}", flush=True)
        return result
...
def health():
    ...
    return jsonify({
        ...
        "use_seamless_clone": settings["use_seamless_clone"],
...
def generate():
    ...
    try:
        with tempfile.TemporaryDirectory(dir=work_dir) as tmp_dir:
            ...
            # Run OPTIMIZED SadTalker
            t0 = time.time()
            current_video = _run_sadtalker_optimized(
                cfg,
            ...
            _log_timing("sadtalker", time.time() - t0)
            ...
            # Print summary
            print("\n" + "="*50)
            print("TIMING SUMMARY (OPTIMIZED):")
...
if __name__ == "__main__":
    print("="*60)
    print("LIPSYNC Server OPTIMIZED")
    print("  FP16 + optional seamlessClone bypass")
    print("  Expected speedup: ~2x (12s -> 6s)")
    print("="*60)
    cfg = _load_config()
